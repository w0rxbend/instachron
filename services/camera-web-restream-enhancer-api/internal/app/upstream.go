...
// upstreamReader connects to the origin's MJPEG stream for one camera, decodes
// each frame through the enhancement pipeline, and pushes the result to the
// local hub. It self-retries with exponential backoff capped at upstreamMaxBackoff.
...
func newUpstreamReader(originURL, cameraID string, hub *cameraHub, enh *enhancer, logger *log.Logger, transport *http.Transport) *upstreamReader {
