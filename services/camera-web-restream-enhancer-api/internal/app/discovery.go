...
func newDiscovery(originURL string, manager *hubManager, enh *enhancer, logger *log.Logger) *discovery {
	transport := &http.Transport{
...
func (d *discovery) poll(ctx context.Context) {
	...
	for _, cam := range cameras {
		...
		if isNew || !d.manager.isUpstreamStarted(cam.ID) {
			...
			ur := newUpstreamReader(d.originURL, cam.ID, hub, d.enh, d.logger, d.transport)
			go ur.run(ctx)
		}
	}
}
