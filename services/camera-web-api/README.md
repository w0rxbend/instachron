...
| --- | --- | --- |
| `GET` | `/` | Browser viewer (embedded from `shared/webui`) |
...
```sh
go build -o camera-web-api ./cmd/camera-web-api
./camera-web-api
```
...
| File | Responsibility |
| --- | --- |
| `cmd/camera-web-api/main.go` | Process entry point |
| `internal/app/run.go` | Env config, signal handling, wires hub manager, socket reader, HTTP server |
| `internal/app/socket_reader.go` | IPC Unix socket client with automatic reconnect; dispatches FRAME and OFFLINE messages |
| `internal/app/hub.go` | Per-camera push hubs; `hubManager` owns liveness checks and camera registry |
| `internal/app/handler.go` | HTTP routes, MJPEG frame writer, embedded web UI |
| `internal/app/rotation.go` | Config file loading, angle normalisation, JPEG decode/rotate/encode pipeline |
