...
// upstreamReader connects to the origin's MJPEG stream for one camera,
// reads raw JPEG frames, and submits them to the worker pool for FSRCNN
// upscaling. It self-retries with exponential backoff.
...
func newUpstreamReader(originURL, cameraID string, hub *cameraHub, pool *workerPool, logger *log.Logger, transport *http.Transport) *upstreamReader {
