...
| File | Responsibility |
| --- | --- |
| `main.go` | Entry point — env config, signal handling, wires server and publisher |
| `server.go` | TCP listener, per-connection goroutines, frame validation, offline notification |
| `protocol.go` | Binary header parsing for JPGS/JPGD variants, JPEG SOI/EOI marker check |
| `publisher.go` | Unix socket IPC server, per-consumer buffered fan-out |
